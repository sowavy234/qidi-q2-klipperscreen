...
def compile_controls(payload):
    ...
    controls = [
        {
            "id": f"filament-{action}",
            "action": action,
            "macro": macro,
            "available": macros.get(action) is True,
        }
        for action, macro in CONTROL_ACTIONS.items()
    ]
    return json.dumps(
        {
            "schema": 1,
